...
def add_message(
    db: Session, session_id: str, role: str, content: str, image_url: str | None = None
) -> Message:
    msg = Message(
        id=str(uuid.uuid4()), session_id=session_id, role=role, content=content, image_url=image_url
    )
    db.add(msg)
    db.commit()
    db.refresh(msg)
